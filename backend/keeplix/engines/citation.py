...
def parse_response(
    prompt: str, sample_index: int, answer_text: str, cited_urls: list[str],
    brand_name: str, aliases: list[str], brand_domains: list[str],
) -> SampleParse:
    names = [brand_name, *aliases]
    mentioned = any(n and n in answer_text for n in names)
    own_cited = any(
        any(d in (urlparse(u).netloc or u) for d in brand_domains) for u in cited_urls
    )
        ...
        for i in range(samples):
            ...
            parsed.append(
                parse_response(
                    ...
                    brand_name, aliases, brand_domains,
                )
            )

    n = len(parsed) or 1
    entity_sov = sum(p.brand_mentioned for p in parsed) / n
    citation_sov = sum(p.own_domain_cited for p in parsed) / n
