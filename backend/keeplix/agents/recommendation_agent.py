...
_ADVICE: dict[str, tuple[str, str, str, str]] = {
    "http_ok": (
        ...
        "确保页面返回 200 且可抓取",
        "检查服务器状态码、robots 是否放行 AI 爬虫。",
        "high",
    ),
    "ssr_content_visible": (
        "technical_crawlability",
        "让正文在无 JS 时可见（SSR/静态）",
        "AI 爬虫多不执行 JS，关键内容需服务端渲染或静态输出。",
        "high",
    ),
    "has_title": (
        "technical_crawlability",
        "补全页面 title",
        "title 是引擎理解页面主题的首要信号。",
        "medium",
    ),
    "direct_answer_lead": (
        "content_structure",
        "开头直接给出答案",
        "首段用 40–200 字直接回答核心问题，利于被摘录引用。",
        "high",
    ),
    "heading_hierarchy": (
        "content_structure",
        "建立 H1/H2 层级",
        "用清晰的 heading 层级组织内容，便于结构化提取。",
        "medium",
    ),
    "qa_or_list_format": (
        "content_structure",
    ...
    "has_date": ("freshness", "标注发布/更新日期", "明确时间戳，帮助引擎判断时效。", "medium"),
    "recent_signal": ("freshness", "保持内容新鲜", "核心页面按季度刷新，更新统计与案例。", "low"),
    "extractable_facts": (
    ...
    "has_jsonld": (
        "entity_alignment",
        "添加 Schema JSON-LD",
        "用结构化标注声明实体，帮助引擎对齐知识图谱。",
        "medium",
    ),
    "entity_named": (
        "entity_alignment",
        "明确站点/组织实体",
        "补 og:site_name 与 Organization 标注。",
        "low",
    ),
    "links_to_preferred_sources": (
