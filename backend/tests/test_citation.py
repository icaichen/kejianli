...
from keeplix.engines.citation import parse_response
...
async def test_run_sampling_aggregates_sov():
    provider = StubProvider("deepseek", brand_name="keeplix",
                            brand_domains=["keeplix.com"])
    report = await citation.run_sampling(
        provider, ["最好的 GEO 工具", "中文内容优化"],
        brand_name="keeplix", brand_domains=["keeplix.com"], samples=3,
    )
